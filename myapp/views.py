    ...
    if request.method == 'GET':
        ...
        try:
            if not request.POST.get("name"):
                raise Exception("Por favor introduzca un nombre")
            ...
            "Acciones": f"""
                        <div class="form-button-action">
                            ...
                            <button type="button" data-toggle="modal"
                                data-target="#resourcesModalDelete" title="Eliminar"
                                class="btn btn-link btn-danger btn-lg"
                                data-original-title="Eliminar" onclick="formDeleteResource({resource.id})">
...
def projects_json(request):
    ...
    for project in projects:
        ...
        data.append({
            ...
            "actions": f"""
                <div class="form-button-action">
                    <a href="/projects/{project.id}/info/general" class="btn btn-link btn-info btn-lg" title="Ver">
                        <i class="fas fa-eye"></i>
                    </a>
                   <button type="button" title="Edit Project"
                        ...
                        onclick="formProjectEdit({project.id})">
                        <i class="fa fa-edit"></i>
                    </button>
                    <button type="button" data-toggle="modal"
...
def project_info(request, id):
    ...
    activities = Activity.objects.all().order_by("etapa_id","id")
    documents = Document.objects.filter(project_id=id).order_by("id")
...
def activity_create(request):
    ...
    if request.method == "POST":
        if not request.POST.get("title") or not request.POST.get("stageSelect"):
            return JsonResponse(
                {"success": False, "error": "El título y etapa son obligatorios."}
            )
        else:
            ...
            return JsonResponse({"status": "success", "message": "Actividad creada correctamente."})
...
@login_required
def activity_resources_delete(request,id_resource):
    resource_multimedia = get_object_or_404(Multimedia,pk=id_resource)
    if request.method == 'POST':
        resource_multimedia.delete()
        return JsonResponse({"success": True, "message": "Recursos multimedia eliminado correctamente."})
    else:
        return render(request,'tasks/activities/resources/resource_delete.html',{                           
            'resource': resource_multimedia
        })
...
@login_required
def project_document_delete(request, id_document):
    if request.method == 'POST':
        document = get_object_or_404(Document, pk=id_document)
        document.delete()
...
def calendar_event_edit(request, id_meeting):
    ...
    if request.method == 'POST':
        try:
            ...
            meeting.notes = request.POST.get('notas')
